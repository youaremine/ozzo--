...
class StoreProductPresell extends BaseController
{
    ...
    public function lst()
    {
        ...
        $where = $this->request->params(['product_status','keyword','status','type','presell_type','mer_id','is_trader','us_status','star','product_presell_id']);
...
//        ], $ret->mer_id);
    ...
    public function productStatus()
    {
        ...
        if($data['status'] == 1){
            ...
            $title = '审核结果';
            $message = '审核通过';
            $type = 'product_presell_success';
        }
        if($data['status'] == -1){
            $title = '审核结果';
            $message = '审核失败';
            $type = 'product_presell_fail';
        }
        if($data['status'] == -2){
            $title = '下架提醒';
            $message = '被下架';
        ...
        SwooleTaskService::merchant('notice', [
            ...
            'data' => [
                'title' => $title,
                'message' => '您有一个预售商品'. $message,
                'id' => $id[0],
                'type' => $ret['presell_type'],
            ]
        ], $ret->mer_id);
        foreach ($id as $item){
